...
#            ana_sol = analytic_sol([1/m**2 for m in list_m], k_u_init, x, nu, dt*N_step)
        ...
        np.savetxt('../data/diffusion/varying_der/'+filename, data, header='x    u_init' + list_nu_str)
    return

def tau_diff_study(int_method, u, F, get_data = False, fname = None):
    #define input variable
    # spatial inputs
    L = 10 # Spatial size of the grid
    ...
    N_step = 300000 # Number of Temporal steps
...
#    print(f'r_c: {r_c}')
    ...
    list_m = [1, 3, 15, 25, 50, 90]
    data_to_fit = np.empty((len(list_m) + 1, N_step))
    data_to_fit[0] = t
    ax.plot([], [], ' ', label=fr"$\nu$ = {nu}")
...
if __name__ == '__main__':
    ...
    derivative_effect(Int.euler_step, u_sin_simple, F_diffusion_der)
    #tau_diff_study(Int.RK2, u_sin_simple, F_diffusion, get_data = True, fname = 'data/fit_diffusion_diff_fin_simm.txt')
